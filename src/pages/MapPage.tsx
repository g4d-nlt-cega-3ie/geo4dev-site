...
import { search, taxonomy, CONTENT_TYPES, bySlug } from '../lib/catalog'
...
export default function MapPage() {
  ...
  const selected = selectedId ? bySlug.get(selectedId) : undefined
  ...
  // honor ?focus=<slug> coming from a library "Map it": center the map on it,
  // show its details on the right, and set its category on the left.
  useEffect(() => {
            ...
            {taxonomy.map((g) => (
              <label key={g.category} className="mc-radio">
                <input
                  ...
                  onChange={() => toggle(categories, setCategories, g.category)}
                />
                {g.category}
              </label>
            ))}
            ...
            {CONTENT_TYPES.map((ct) => (
              <label key={ct} className="mc-radio">
                <input
                  ...
                  onChange={() => toggle(contentTypes, setContentTypes, ct)}
                />
                {ct}
              </label>
            ))}
            {(categories.length > 0 || contentTypes.length > 0 || q) && (
              <button className="mp-clear" onClick={() => { setQ(''); setCategories([]); setContentTypes([]) }}>
                Clear filters
              </button>
            )}
          </Section>
            ...
            <div className="mp-list">
              {results.map((d) => {
                ...
                return (
                  <button
                    key={d.id}
                    className={`mp-item ${selectedId === d.id ? 'active' : ''}`}
                    onClick={() => (geo ? focus(d) : setSelectedId(d.id))}
                  >
        ...
        <aside className="map-pane right">
          {selected ? (
            ...
            <div className="mp-empty">
              <p className="muted">Select a point on the map or a result on the left to see its details here.</p>
            </div>
          )}
        </aside>
      </div>
